...
public final class PowerModelConstants {
	...
	static {
		...
		final Resource resource = resourceSet.createResource(URI.createURI(PATHMAP_POWER_MODEL_MODELS_COMMON_POWER_MODELS_POWERMODEL, true));
		try {
		...
		// TODO load models per hierarchy, not per ID for nested elements/attributes?
		...
		LINEAR_PASSTHROUGH_DISTRIBUTION = (DistributionPowerModelSpecification) resource.getEObject("_EzkPUDqsEeSeEKsoUNmaIw");
		LINEAR_PASSTHROUGH_DISTRIBUTION_CONSTANT_LOSS = (ConsumptionFactor) resource.getEObject("_b_IXADqsEeSeEKsoUNmaIw");
